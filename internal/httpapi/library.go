...
func (s *Server) adminLibraryCDKRevoke(w http.ResponseWriter, r *http.Request, id string) {
	p, ok := s.authAdmin(w, r)
	if !ok || !checkAdminCSRF(w, r) {
		return
	}
	if err := s.App.RevokeLibraryCDK(r.Context(), id, p.Admin.ID); err != nil {
		writeError(w, r, err)
		return
	}
...
func (s *Server) adminQuestionBankDelete(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err := s.App.DisableQuestionBank(r.Context(), id, p.Admin.ID); err != nil {
		writeError(w, r, err)
		return
	}
